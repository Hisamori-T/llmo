...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiClient.post('/auth/signup', { id_token: idToken, display_name: name });
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      const idToken = await credential.user.getIdToken();
      const res = await apiClient.post('/auth/signup', { id_token: idToken, display_name: credential.user.displayName ?? '' });
      localStorage.setItem('session_id', res.data.session_id);
      toast.success('アカウントを作成しました');
      router.push('/dashboard');
    } catch {
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex items-center justify-center px-4">
      <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm w-full max-w-md">
        ...
        <button onClick={handleGoogleSignup} disabled={loading} className="w-full inline-flex items-center justify-center gap-2 h-10 px-4 text-[1rem] font-medium bg-white text-slate-700 border border-slate-200 rounded-lg hover:bg-gray-50 cursor-pointer mb-6 disabled:opacity-50">
